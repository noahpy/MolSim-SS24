...
\begin{itemize}
    ...
    \item The measurements were conducted on a large scenario ($\geq 10000\ molecules$) with I/O operations disabled. The gathered data is presented in Tab.\ \ref{table:data}.
    \item The data indicates that the force calculation routine is the most time-consuming section, accounting for 77.07\% of the total runtime (\texttt{lj\_calc(...)} is part of force calculation).
\end{itemize}
...
\begin{table}[h!]
    ...
    \caption{Performance measurements. Each sample counts as 0.01 seconds.}
    \label{table:data}
\end{table}

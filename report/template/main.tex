...
\begin{itemize}
    ...
    \item \emph{Please not, that we are aware that the documentation cannot be built on the branch that we created the initial pull request for, as the documentation was only added with another branch}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item We ware eager to see, how other groups managed to find a solution
    \end{itemize}
...
\begin{itemize}
    ...
    \item This also allows to use range based for loops to loop over the particles (by accessing the vector)
    ...
    \item This is useful for example when calculating the forces in one simulation step, as you need to access each pair
\end{itemize}

\subsection{Strategy Pattern}
\begin{itemize}
    \item To allow high flexibility regarding the approach of calculating the forces, velocities, and positions, we opted to implement the strategy pattern
...
\begin{itemize}
    ...
    \item This is many focused on the \verb|ParticleContainer| class and its iterators

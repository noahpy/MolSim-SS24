 ...
 */
typedef std::map<std::vector<Position>, std::vector<PeriodicBoundShifts>> MultiDimPeriodicBoundShiftsMap;
...
public:
     ...
     */
    PeriodicBoundary(Position position, const BoundaryConfig& boundaryConfig, const CellGrid& cellGrid);
...
private:
    ...
    PeriodicBoundShifts innerTranslation; /**< The translations to apply to cells that are only affected by one boundary */
    MultiDimPeriodicBoundShiftsMap translationMap; /**< The translations to apply to cells that are only affected by two boundaries */
     ...
     * @return The shifts
     */
    PeriodicBoundShifts getPeriodicShift(const CellGrid& grid) { return getPeriodicShiftFromPosition(position, grid); }
    /**
    ...
    PeriodicBoundShifts getPeriodicShiftFromPosition(Position pos, const CellGrid& grid);
      ...
      */
    void fillTranslationMap(const std::vector<Position>& key, const std::vector<Position>& boundarySides, const CellGrid& cellGrid);
};

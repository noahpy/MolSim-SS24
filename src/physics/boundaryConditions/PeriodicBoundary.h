...
private:
    ...
    /**
     * @brief This method checks, if the side is responsible for the shifts. We need this, as we only want to shift particles once. E.g. if a cell belongs to two boundaries, we only want to shift it once. For that we check, if the side is the highest priority periodic side
     ...
     */
    bool isThisSideResponsibleForShifts(const BoundaryConfig& boundaryConfig, const std::vector<Position>& boundarySides) const;
};

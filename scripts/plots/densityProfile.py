...
doPickle = False # When true you must transpose the data matrix
...
if not doPickle:
    ...
    with open('./results/out_file_density.pkl', 'rb') as f:
        density = pickle.load(f)
    with open('./results/out_file_mean.pkl', 'rb') as f:
...
ani.save('density_profile.mp4', writer=writer)
plt.show()
